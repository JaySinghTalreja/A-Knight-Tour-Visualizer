class KnightsTour {
  constructor(cols = 8, rows = 8) {
    ...
    this.drawKnight = this.drawKnight.bind(this);
    ...
    for (let row = 0; row < rows; row += 1) {
      ...
      for (let col = 0; col < cols; col += 1) {
        ...
        grid[row][col] = {
          ...
          color: col % 2 === row % 2 ? '#FFFFFF' : '#000000',
        };
  ...
  goOnTour(startPos) {
    ...
    // repeat
    console.log("FINAL:"+nextSquare.pos);
  ...
  drawBoard() {
    ...
    // gutter
    this.drawRectBase(0, 0, gutterX * sqSize, gutterY * sqSize, '#696969');
    // file labels
    ...
    grid.map(row => row.map(col => {
      ...
      //this.drawRectBase(offsetX, offsetY, sqSize, sqSize, col.color);
      return false;
    }));
  }
  drawPath(path) {
  ...
  drawRectBase(x, y, width, height, color) {
    const { ctx } = this;
    ctx.fillStyle = color;
    ctx.fillRect(x, y, width, height); 
  ...
  drawKnight(value, x, y, font = '16px sans-serif') {
    const { ctx } = this;
    ctx.font = font;
    ctx.textAlign = 'center';
    ctx.textBaseline = 'middle';
